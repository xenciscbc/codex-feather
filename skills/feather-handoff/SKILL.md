...
3. **Resume:** read the full work and use [source comparison](references/snapshots.md) when it has a baseline. Interpret changed and unknown observations against relevant sources before acting; an absent baseline uses the existing manual checks. Correct outdated facts, perform the currently authorized next step, and maintain the same handoff. Identify missing requirements; continue independent authorized work where possible. Ordinary read/list requests do not compare sources or execute next steps.
