...
Git HEAD and branch are shown independently of file observations. Detached HEAD has `branch: null`; an unborn branch has `head: null`; non-Git projects can still have complete file comparisons. Unknown Git information leaves the overall result incomplete without hiding file results. Probe commands use process-scoped `safe.directory` for the selected root, consistent with existing root discovery; they do not modify Git configuration.
