...
互動預檢會分別回報專案與使用者範圍；某個範圍的紀錄無法讀取時，仍列出另一範圍的結果並讓使用者選擇操作。實際操作仍核對所有相關的安裝紀錄：選中的範圍或其相依來源有問題時停止寫入，不把預檢錯誤當作未安裝。錯誤的專案路徑或素材包會在選擇操作前停止。
...
獨立 handoff 的安裝、更新與遷移也會核對這些可見範圍的 plugin handoff 紀錄。Plugin 維護入口即使沒有獨立 skill 檔案，仍屬於既有來源；需先明確管理原安裝，不能在另一範圍建立重複副本。互動流程在實際套用前重新執行範圍檢查，若出現 plugin 來源衝突便停止。
