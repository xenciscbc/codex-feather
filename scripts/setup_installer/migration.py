...
def migrate(environment: Environment, bundle: Bundle, components: list[str], source_scope: str,
            ...
            expected_plan: str | None = None, handoff_provider: Path | None = None) -> dict:
    ...
    provider = plugin_provider(handoff_provider) if "handoff" in selected else None
    ...
    for component in selected:
        ...
        if not record or record.get("reused") or (not record.get("files") and not record.get("provider")):
            raise ValueError(f"No owned migration source: {component} in {source_scope}")
        if record.get("provider") and not provider:
            raise ValueError("Plugin handoff provider is unavailable; migrate with the owning plugin")
