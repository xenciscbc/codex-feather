...
01–07 已完成；各票 Answer 記錄來源 CLI、Windows／Linux 執行檔、原生 Codex 與故障復原證據。08 功能已完成，兩平台發行候選包各 52 項情境通過；完整專案來源回歸 88 項通過（條件略過另列），正在做程式碼審查及最終交付包收尾。
...
- 08 剩餘工作：依已確認基準 fc40236 進行 Standards／Spec 雙軸審查、處理發現、建置並驗證最終交付包，以及提交驗證與審查紀錄。
