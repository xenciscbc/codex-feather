...
class PluginSetupTest(unittest.TestCase):
    ...
    def test_explicit_handoff_remove_cleans_legacy_installation_only(self):
        ...
        command = [sys.executable, "-B", str(self.plugin / "scripts/feather_setup.py"), "install",
                   ...
                   "--bundle", str(bundle), "--components", "handoff", "--json"]
    ...
    def test_wrapper_rejects_handoff_install_all_install_and_missing_project(self):
        before = self.target_files()
        for args in (("install", "--components", "handoff"),
            ...
            with self.subTest(args=args):
                ...
                self.assertNotEqual(result.returncode, 0)
                self.assertIn("Only delegation" if "--components" in args else "plugin supplies its own bundle",
                              result.stderr)
        missing = self.run_setup("install", project=False)
